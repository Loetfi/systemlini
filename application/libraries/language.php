...
class Language {
    ...
    public function from_body() {
        ...
        $active_laguage = 'English';
        // get active language info
        $form_info = $CI->db->query("SELECT form_id FROM tbl_form ORDER BY form_id");
        // while ($form_id = mysqli_fetch_assoc($form_info)) {
        foreach ($form_info->result_array() as $form_id) {
            $id[] = $form_id;
        }
        foreach ($id as $v_id) {
            $f_id = $v_id['form_id'];
            ...
            foreach ($user_menu->result_array() as $form_data) {
