...
class Menu {
    ...
    public function dynamicMenu() {
        ...
        if ($user_type != 1) {// query for employee user role            
            ...
            $user_menu = $CI->db->query("SELECT * FROM tbl_menu ORDER BY sort");
        }
        $user_language = $CI->db->query("SELECT active_language FROM tbl_gsettings ORDER BY id_gsettings");
        ...
        foreach ($user_language->result_array() as $active_laguage) {
