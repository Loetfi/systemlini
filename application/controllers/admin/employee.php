...
class Employee extends Admin_Controller {
    ...
    public function save_employee($id = NULL) {
        ...
        $data = $this->employee_model->array_from_post(array('first_name', 'last_name', 'date_of_birth', 'gender', 'maratial_status', 'father_name', 'nationality',
            'passport_number', 'address', 'present_address', 'city', 'country_id', 'mobile', 'phone', 'email', 'employment_id', 'designations_id', 'joining_date'));
        //image upload
        ...
        // ** Employee Education Information Save and Update Start  **
        $education_data = $this->employee_model->array_from_post(array('elementary_school', 'junior_high_school', 'high_school', 'diplome', 'university'));
        $education_data['employee_id'] = $employee_id;
        $this->employee_model->_table_name = "tbl_employee_education"; // table name
        $this->employee_model->_primary_key = "employee_education_id"; // $id
        $employee_education_id = $this->input->post('employee_education_id', TRUE);
        if (!empty($employee_education_id)) {
            $this->employee_model->save($education_data, $employee_education_id);
        } else {
            $this->employee_model->save($education_data);
        }
        // * Employee Education Information Save and Update End   *

        // ** Employee Family Information Save and Update Start  **
        $family_data = $this->employee_model->array_from_post(array('spouse_name', 'child_name', 'child2_name', 'mother_name', 'own_brosis_name', 'own_brosis_name1', 'current_address'));
        $family_data['employee_id'] = $employee_id;
        $this->employee_model->_table_name = "tbl_employee_family"; // table name
        $this->employee_model->_primary_key = "employee_family_id"; // $id
        $employee_family_id = $this->input->post('employee_family_id', TRUE);
        if (!empty($employee_family_id)) {
            $this->employee_model->save($family_data, $employee_family_id);
        } else {
            $this->employee_model->save($family_data);
        }
        // * Employee Family Information Save and Update End   * 

        // ** Employee Work Experience Information Save and Update Start  **
        $experience_data = $this->employee_model->array_from_post(array('company_name', 'position', 'job_desk', 'duration', 'salary', 'reason'));
        $experience_data['employee_id'] = $employee_id;
        $this->employee_model->_table_name = "tbl_employee_experience"; // table name
        $this->employee_model->_primary_key = "employee_experience_id"; // $id
        $employee_experience_id = $this->input->post('employee_experience_id', TRUE);
        if (!empty($employee_experience_id)) {
            $this->employee_model->save($experience_data, $employee_experience_id);
        } else {
            $this->employee_model->save($experience_data);
        }
        // * Employee Work Experience Information Save and Update End   * 

        // ** Employee Skill Information Save and Update Start  **
        $skill_data = $this->employee_model->array_from_post(array('technical', 'computer', 'english'));
        $skill_data['employee_id'] = $employee_id;
        $this->employee_model->_table_name = "tbl_employee_skill"; // table name
        $this->employee_model->_primary_key = "employee_skill_id"; // $id
        $employee_skill_id = $this->input->post('employee_skill_id', TRUE);
        if (!empty($employee_skill_id)) {
            $this->employee_model->save($skill_data, $employee_skill_id);
        } else {
            $this->employee_model->save($skill_data);
        }
        // * Employee Skill Information Save and Update End   *
