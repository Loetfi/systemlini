...
class Dashboard extends Employee_Controller {
    ...
    ###
    public function print_apply_leave_application($id_leave) {
        ...
        $getdata = $this->db->query("SELECT b.category ,d.designations, a.* , c.first_name, c.last_name , c.employment_id FROM tbl_application_list a 
            INNER JOIN tbl_leave_category b ON a.leave_category_id = b.leave_category_id
            INNER JOIN tbl_employee c ON a.employee_id = c.employee_id
            INNER JOIN tbl_designations d ON c.designations_id = d.designations_id
            WHERE application_status = 2 AND a.application_list_id = $id_leave")->row();

        $data['print'] = $getdata;

        // $data['subview'] = $this->load->view('employee/print_leave', $data, TRUE);
        $this->load->view('employee/print_leave', $data);
    }
